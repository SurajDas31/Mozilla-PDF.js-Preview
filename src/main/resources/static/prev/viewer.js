...
var PDFViewerApplication = {
  ...
  url: '',
  //documentAnnotations : null, //Added by Bane Singh
  //rubberStampAnnotation : null, //Added by Bane Singh
  //deleteConfirmationPrompt : null,//Added by Bane Singh
  //stickyNotePrompt : null,//Added by Bane Singh
  //deleteAnnotationConfirmationPrompt : null,//Added by Bane Singh
  //signDocumentDialog : null,//Added by Bane Singh
  baseUrl: '',
      ...
      return _regenerator2.default.wrap(function _callee5$(_context5) {
        while (1) {
          switch (_context5.prev = _context5.next) {
			...
			// 	container: document.getElementById('rubberStampOverlay'),
			// });
			...
			this.deleteAnnotationConfirmationPrompt = new DeleteAnnotationConfirmationPrompt({overlayName: 'confirmDeleteAnnotationOverlay',
				...
				overlayManager : this.overlayManager,
				container: document.getElementById('confirmDeleteAnnotationOverlay'),
			});
			...
			this.stickyNotePrompt = new StickyNotePrompt({overlayName: 'stickyNoteOverlay',
				...
				overlayManager : this.overlayManager,
				container: document.getElementById('stickyNoteOverlay'),
			});
			//Added by Bane Singh
			this.signDocumentDialog = new SignDocumentDialog({overlayName: 'signDocumentOverlay',
				closeButton: document.getElementById('signDocumentOverlayCancel'),
				clearButton: document.getElementById('signDocumentOverlayClear'),
				okButton: document.getElementById('signDocumentOverlayOK'),
				browseButton: document.getElementById('signDocumentOverlayBrowse'),
				overlayManager : this.overlayManager,
				container: document.getElementById('signDocumentOverlay'),
			});
  ...
  setInitialView: function setInitialView(storedHash) {
    ...
    this.secondaryToolbar.setPageNumber(this.pdfViewer.currentPageNumber);
     PDFViewerApplication.loadDocumentAnnotations(); //Bane Singh
  ...
  loadDocumentAnnotations : function pdfLoadDocumentAnnotations(){
			 ...
			 var annotationUrl = "../pdfannotation/viewer?documentid="+docId+"&revisionid="+revisionid;
			 $.ajax({
				 async : false,
				 type:"GET",
				 url  : annotationUrl,
				 success : function (response){
					 PDFViewerApplication.documentAnnotations = response;
				 },
		...
		saveAndLoadDocumentAnnotations: function pdfSaveAndLoadDocumentAnnotations(){
			 ...
			 for(var i = 0 ; i < PDFViewerApplication.documentAnnotations.length ; i++){
				 var annotation = PDFViewerApplication.documentAnnotations[i];
				 if(annotation.annotationId == 0 ){ //newly created annotation so check if it is deleted or not
					 if(annotation.ddeleted){
			 ...
			 $.ajax({
				 ...
				 data: annotations,
				 url  : "../pdfannotation/viewer?documentid="+docId+"&revisionid="+revisionid+"&sid="+sid,
				 success : function (response){
					 PDFViewerApplication.documentAnnotations = response;
				 },
				 error: function (jqXHR, exception) {
	  ...
	  drawDocumentAnnotations : function pdfDrawDocumentAnnotations(pageView,annCtx,pageNumber){
			...
			annCtx.clearRect(0,0,annCtx.canvas.width,annCtx.canvas.height); // We need to clear complete canvas here
			if(pageView.rotation == 90 ){
				annCtx.translate(annCtx.canvas.width , 0 );
			}else if(pageView.rotation == 180){
				annCtx.translate(annCtx.canvas.width , annCtx.canvas.height );
			}else if(pageView.rotation == 270){
				annCtx.translate(0, annCtx.canvas.height );
			}else{
			...
			for(var i =0 ; i < PDFViewerApplication.documentAnnotations.length; i++){
				...
				if(pageNumber == documentAnnotation.pageId){
					if(!documentAnnotation.ddeleted){
			...
			for(var i = 0 ; i < PDFViewerApplication.documentAnnotations.length;i++){
				...
				if(!documentAnnotation.ddeleted){
					...
					html+= "<div class='username'>"+documentAnnotation.realName+"</div>";
					...
					}else if(documentAnnotation.annotationType === ANNOTATION_SIGNATURE){
						html+= "<div class='annotation annotation-signature'></div>";
					}else{
						//html+= "<div class='annotation annotation-rubberstamp' title='"+documentAnnotation.text+"'></div>";
					}
					html+= "</div>";
					html+= "</a>";
		...
		signDocument : function pdfSignDocument(){
			PDFViewerApplication.signDocumentDialog.open();
		},
...
function webViewerInitialized() {
  ...
  document.getElementById('saveAnnotations').addEventListener('click',function() {
	    $('.canvasWrapper').eq(0).show();
		$('.canvasWrapper').eq(1).show();
	...
	document.getElementById('highlighttext').addEventListener('click',function() {
          ...
          if((objOffsetVersion=objAgent.indexOf("Firefox"))!=-1){ }
			 ...
			 $('.canvasWrapper').eq(0).hide();
		     $('.canvasWrapper').eq(1).hide();  
		  }
	});
   document.getElementById('signDocument').addEventListener('click',function() {
		$('.canvasWrapper').eq(0).show();
		$('.canvasWrapper').eq(1).show();
		PDFViewerApplication.signDocument();
	});
  try {
...
function webViewerPageRendered(evt) {
  ...
  //Added by Bane Singh for drawing custom annotations
  var objAgent=navigator.userAgent;
  var objOffsetVersion;
  if((objOffsetVersion=objAgent.indexOf("Firefox"))!=-1){
	...
	document.getElementById('signDocument').disabled = true;
	...
	//document.getElementById('annotationsToolbar').style.display='none';
